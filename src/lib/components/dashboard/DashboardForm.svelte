<script lang="ts">
	...
	export let formType: 'updateProfile' | 'newAddress' | 'updateAddress';
...
>
	...
	{#if formType === 'newAddress' || formType === 'updateAddress'}
		<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
			<div class="w-full">
				<label for="first_name" class="label">
					<span class="label-text">{$LL.account.auth.firstName()}</span>
			...
			<div class="w-full">
				...
				<select class="select select-bordered w-full" name="country" bind:value={addressCountry}>
